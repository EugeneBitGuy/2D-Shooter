...
public class InputManager : MonoBehaviour
{
    ...
    public void ReadDodgeInput(InputAction.CallbackContext context)
    {
        dodgeHeld = !context.canceled;
        //StartCoroutine(ResetDodgeStart());
    }

    private IEnumerator ResetDodgeStart()
    {
        yield return new WaitForEndOfFrame();
        dodgeHeld = false;
    }

    public void ReadThrowingGrenadeInput(InputAction.CallbackContext context)
    {
        grenadeThrowingHeld = !context.canceled && context.interaction is HoldInteraction;

        if(!context.performed)
            return;
        
        grenadeThrowingPressed = context.interaction is PressInteraction;
        StartCoroutine(ResetGrenadeThrowingStart());
    }

    public bool grenadeThrowingHeld;
    

    private IEnumerator ResetGrenadeThrowingStart()
    {
        yield return new WaitForEndOfFrame();
        grenadeThrowingPressed = false;
    }

    public bool grenadeThrowingPressed;
    ...
    public void ReadSuperFireInput(InputAction.CallbackContext context)
    {
        superFirePressed = !context.canceled;
        StartCoroutine("ResetSuperFireStart");
    }

    IEnumerator ResetSuperFireStart()
    {
        yield return new WaitForEndOfFrame();
        superFirePressed = false;
    }
