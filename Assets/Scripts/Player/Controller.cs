...
public class Controller : MonoBehaviour
{
    ...
    [Tooltip("Gun gameobject that player uses right now")]
    public GameObject currentGun;

    public GrenadeThrowingContorller grenadeLauncher;
    ...
    private void Start()
    {
        ...
        var gunsNumber = currentGun != null ? currentGun.GetComponent<MultiGunShootingController>().guns.Length : 0;
        
        playerAnimator.SetInteger("numberofGuns", gunsNumber);
        SetupColliderInfo(gunsNumber);
    }
    ...
    private void MovePlayer(Vector3 movement)
    {
        ...
        // Don't move in the x if the settings stop us from doing so
            ...
            //myRigidbody.AddForce(movement */* Time.deltaTime **/ moveSpeed);
            myRigidbody.MovePosition(myRigidbody.position + (Vector2) movement * Time.fixedDeltaTime * moveSpeed);

            if (inputManager.dodgeHeld && movement != Vector3.zero && Time.timeSinceLevelLoad - lastDodgeTime > dodgeCooldown)
            {
                StartCoroutine(Dodge(movement));
            }
            
            playerAnimator.SetBool("isWalking", movement != Vector3.zero);
        
    }

    private bool isDodging;
    private IEnumerator Dodge(Vector2 movement)
    {
        tr.emitting = true;
        isDodging = true;
        myRigidbody.velocity = movement * dodgeSize;
        yield return new WaitForSeconds(0.3f);
        myRigidbody.velocity = Vector2.zero;
        isDodging = false;
        lastDodgeTime = Time.timeSinceLevelLoad;
        tr.emitting = false;
    }
    ...
    public void ReplaceGun(GameObject gun)
    {
        if(gun.GetComponent<ShootingController>() == null)
        {
            return;
        }
        
        Destroy(currentGun);
        var newGun = Instantiate(gun, gameObject.transform);
        currentGun = newGun;
        var gunsNumber = newGun.GetComponent<MultiGunShootingController>().guns.Length;
        playerAnimator.SetInteger("numberofGuns", gunsNumber) ;
        SetColliderSize(gunsNumber);
    ...
    private Vector2[][] colliderShapeStates;

    private void SetColliderSize(int numberofGuns)
    {
        ...
        col.pathCount = 1;
        col.SetPath(0, colliderShapeStates[numberofGuns]);

        var fairestPathEdge = colliderShapeStates[numberofGuns].Max(point => point.y);
        grenadeLauncher.transform.localPosition = new Vector3(0, fairestPathEdge + 1, 0);
    }

    private void SetupColliderInfo(int numberofGunsAtStart)
    {
        ...
        if (col != null)
        {
            if (col.pathCount >= 3)
            {
                var colliderShapeWithoutGuns = col.GetPath(0);
                var colliderShapeWithOneGun = col.GetPath(1);
                var colliderShapeWithTwoGuns = col.GetPath(2);

                colliderShapeStates = new[]
                    {colliderShapeWithoutGuns, colliderShapeWithOneGun, colliderShapeWithTwoGuns};

                col.pathCount = 0;
                
                SetColliderSize(numberofGunsAtStart);
            }
        }
